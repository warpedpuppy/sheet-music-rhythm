import { render, screen, waitFor } from '@testing-library/react'
import userEvent from '@testing-library/user-event'
import { MemoryRouter, Route, Routes } from 'react-router-dom'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { api } from '../api/client'
import type { AttemptResult, Exercise } from '../api/types'
import { AuthProvider } from '../auth/AuthContext'
import { ExercisePlayer } from './ExercisePlayer'

// The audio engine is replaced with a stub that finishes playback immediately,
// so the player flows straight from "Start" to scoring without real audio.
vi.mock('../lib/audio', () => {
  class FakeAudioEngine {
    start(_ticks: unknown, handlers: { onDone?: () => void } = {}) {
      handlers.onDone?.()
      return 0
    }
    stop() {}
    dispose() {}
  }
  return {
    AudioEngine: FakeAudioEngine,
    buildExerciseTicks: () => [],
    buildPlaybackTicks: () => [],
  }
})

const exercise: Exercise = {
  id: 1,
  title: 'Steady Quarters',
  difficulty: 1,
  time_signature: '4/4',
  tempo_bpm: 60,
  num_measures: 1,
  pattern: { events: [{ type: 'note', duration: 'q' }, { type: 'note', duration: 'q' }, { type: 'note', duration: 'q' }, { type: 'note', duration: 'q' }] },
  concept_tags: ['quarter-notes'],
  learn_section_slug: 'note-values',
  count_in_beats: 4,
  tolerance_ms: 250,
  user_status: null,
}

const passedResult: AttemptResult = {
  attempt_id: 10,
  gave_up: false,
  results: [
    { index: 0, expected_ms: 4000, status: 'hit', tap_ms: 4000, delta_ms: 0 },
    { index: 1, expected_ms: 5000, status: 'hit', tap_ms: 5010, delta_ms: 10 },
    { index: 2, expected_ms: 6000, status: 'late', tap_ms: 6200, delta_ms: 200 },
    { index: 3, expected_ms: 7000, status: 'missed', tap_ms: null, delta_ms: null },
  ],
  extra_taps: [],
  accuracy: 0.875,
  passed: true,
  tolerance_ms: 250,
  progression: {
    unlocked_level: 1,
    leveled_up: false,
    remediation_started: false,
    remediation_resolved: false,
    suggestion: null,
  },
}

const gaveUpResult: AttemptResult = {
  ...passedResult,
  gave_up: true,
  results: [],
  accuracy: null,
  passed: false,
}

function renderPlayer() {
  return render(
    <MemoryRouter initialEntries={['/exercises/1']}>
      <AuthProvider>
        <Routes>
          <Route path="/exercises/:id" element={<ExercisePlayer />} />
        </Routes>
      </AuthProvider>
    </MemoryRouter>,
  )
}

afterEach(() => {
  vi.restoreAllMocks()
  localStorage.clear()
})

describe('ExercisePlayer', () => {
  it('shows the exercise, instructions, give-up button, and learn link', async () => {
    vi.spyOn(api, 'getExercise').mockResolvedValue(exercise)
    renderPlayer()

    expect(await screen.findByRole('heading', { name: 'Steady Quarters' })).toBeInTheDocument()
    expect(screen.getByText(/tap the/i)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /start/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /i give up/i })).toBeInTheDocument()
    const learnLink = screen.getByRole('link', { name: /read about note values/i })
    expect(learnLink).toHaveAttribute('href', '/learn#note-values')
  })

  it('submits an attempt after the run finishes and shows the score', async () => {
    vi.spyOn(api, 'getExercise').mockResolvedValue(exercise)
    const submitSpy = vi.spyOn(api, 'submitAttempt').mockResolvedValue(passedResult)
    vi.spyOn(api, 'getNextExercise').mockResolvedValue({
      exercise: { ...exercise, id: 2, title: 'Next One' },
      reason: 'progression',
      source_exercise_id: null,
      source_exercise_title: null,
    })
    const user = userEvent.setup()
    renderPlayer()

    await screen.findByRole('heading', { name: 'Steady Quarters' })
    await user.click(screen.getByRole('button', { name: /^start$/i }))

    await waitFor(() => expect(submitSpy).toHaveBeenCalledWith(1, [], false))
    expect(await screen.findByText(/passed!/i)).toBeInTheDocument()
    expect(screen.getByText('88%')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /next exercise/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /try again/i })).toBeInTheDocument()
  })

  it('records a gave-up attempt when the user gives up', async () => {
    vi.spyOn(api, 'getExercise').mockResolvedValue(exercise)
    const submitSpy = vi.spyOn(api, 'submitAttempt').mockResolvedValue(gaveUpResult)
    vi.spyOn(api, 'getNextExercise').mockResolvedValue({
      exercise: null,
      reason: 'completed',
      source_exercise_id: null,
      source_exercise_title: null,
    })
    const user = userEvent.setup()
    renderPlayer()

    await screen.findByRole('heading', { name: 'Steady Quarters' })
    await user.click(screen.getByRole('button', { name: /i give up/i }))

    await waitFor(() => expect(submitSpy).toHaveBeenCalledWith(1, [], true))
    expect(await screen.findByText(/you heard the correct rhythm/i)).toBeInTheDocument()
  })
})
